  ...
  useEffect(() => {
    ...
    if (now - lastCaptureRef.current >= CAPTURE_INTERVAL) {
      const record: Omit<MeasurementRecord, 'id'> = {
        ...
        solar_power: measurements.solar_power ?? null,
        aux_power: measurements.aux_power ?? 0,
        solar_garden_room_power: measurements.solar_garden_room_power ?? null,
        solar_loft_power: measurements.solar_loft_power ?? null,
